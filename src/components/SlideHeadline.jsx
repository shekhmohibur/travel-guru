...
const SlideHeadline = ({title, description, navigate}) => {
    ...
    return (
        <div className="flex gap-5 flex-col ">
            <h2 className="font-bebas text-white text-5xl md:text-6xl">{title}</h2>
            <p className={`text-base-300 font-montserrat text-lg lg:text-2xl ${location.pathname === `/place/${placeId}` ? "" :"line-clamp-3"} md:text-xl text-start`}>{description}</p>
            {
